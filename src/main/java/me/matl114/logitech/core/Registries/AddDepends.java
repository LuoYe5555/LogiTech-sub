...
public class AddDepends {
    ...
    public static void registerSlimefunItems(SlimefunAddon plugin) {
        ...
        try {
            try {
                if (hasNetwork) {
                    SlimefunItem baseItem = SlimefunItem.getById("NTW_QUANTUM_STORAGE_14");
                    ItemStack baseItemStack;
                    Object[] rawRecipe;
                    String storageCapacityText;

                    if (baseItem != null) {
                        // 成功获取到 NTW_QUANTUM_STORAGE_14
                        baseItemStack = baseItem.getItem().clone();
                        //更改介绍
                        storageCapacityText = "&7本物品的最大存储量为140T";
                        // 使用新配方
                        ...
                        // 获取不到 NTW_QUANTUM_STORAGE_14，回退到 NTW_QUANTUM_STORAGE_8
                        baseItem = SlimefunItem.getById("NTW_QUANTUM_STORAGE_8");
                        if (baseItem != null) {
                            baseItemStack = baseItem.getItem().clone();
                            ...
                            // 如果两个都获取不到，抛出异常或处理错误
                            throw new IllegalArgumentException("AN ERROR OCCURED: DON'T FIND NTW_QUANTUM_STORAGE_14 OR NTW_QUANTUM_STORAGE_8");
                        }
                    }
